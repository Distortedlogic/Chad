    ...
    def fitness(self, individual, full_stats=False):
        ...
        for time_index, row in signals.iterrows():
            ...
            if row["buy"] and position >= -1 and position < 1:
                ...
                if (
                    history.empty
                    or history.loc[history["type"] == "short", "closed"].all()
                ):
                    record = {
            ...
            if row["sell"] and position > -1 and position <= 1:
                ...
                if (
                    history.empty
                    or history.loc[history["type"] == "long", "closed"].all()
                ):
                    record = {
                        "entry_time": time_index,
                        "entry": total_sell,
        ...
        mean_entry_time_diff = (
            history["entry_time"].sub(history["entry_time"].shift(1)).dropna().mean()
        )
